...
export default function ParticipantsTab({ competitionId, eventIds, psychSheets }) {
  ...
  async function loadParticipants(pageNumber) {
    ...
    try {
      const response = await fetch(
        getApiUrl(`${process.env.NEXT_PUBLIC_FRONTEND_URL}/participants/client/all/${competitionId}?page=${pageNumber}`),
        {
          credentials: "include",
          cache: "no-store",
        }
  ...
  return (
    <div className="min-w-0 rounded-xl border bg-white p-6 shadow-sm">
      <h2 className="text-2xl font-bold mb-4">Dự đoán của mọi người</h2>
      <div className="pb-2">
